...
RIFF::WAV::File::File(IOStream *stream, bool readProperties,
                      Properties::ReadStyle, ID3v2::FrameFactory *frameFactory)
...
TagLib::Tag *RIFF::WAV::File::tag() const { return &d->tag; }
...
PropertyMap RIFF::WAV::File::properties() const { return d->tag.properties(); }

void RIFF::WAV::File::removeUnsupportedProperties(
...
bool RIFF::WAV::File::save() { return RIFF::WAV::File::save(AllTags); }

bool RIFF::WAV::File::save(TagTypes tags, StripTags strip,
...
bool RIFF::WAV::File::hasID3v2Tag() const { return d->hasID3v2; }

bool RIFF::WAV::File::hasInfoTag() const { return d->hasInfo; }

bool RIFF::WAV::File::hasiXMLTag() const { return d->hasiXML; }

bool RIFF::WAV::File::hasBEXTTag() const { return d->hasBEXT; }
...
void RIFF::WAV::File::read(bool readProperties) {
  for (unsigned int i = 0; i < chunkCount(); ++i) {
    if (const ByteVector name = chunkName(i);
        name == "ID3 " || name == "id3 ") {
      if (!d->tag[ID3v2Index]) {
        d->tag.set(ID3v2Index,
                   new ID3v2::Tag(this, chunkOffset(i), d->ID3v2FrameFactory));
