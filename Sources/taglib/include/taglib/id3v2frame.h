 ...
 * (<a
 * href="https://github.com/taglib/taglib/blob/master/taglib/mpeg/id3v2/id3v2.4.0-frames.txt">
...
public:
  ...
  /*!
   * Returns an appropriate ID3 frame ID for the given free-form tag key. This
   * method will return an empty ByteVector if no specialized translation is
   * found.
   */
  static ByteVector keyToFrameID(const String &);

  /*!
   * Returns a free-form tag name for the given ID3 frame ID. Note that this
   * does not work for general frame IDs such as TXXX or WXXX; in such a case an
   * empty string is returned.
   */
  static String frameIDToKey(const ByteVector &);

  /*!
   * The string with which an instrument name is prefixed to build a key in a
   * PropertyMap; used to translate PropertyMaps to TMCL frames. In the current
   * implementation, this is "PERFORMER:".
   */
  static const String instrumentPrefix;
  /*!
   * The PropertyMap key prefix which triggers the use of a COMM frame instead
   * of a TXXX frame for a non-standard key. In the current implementation, this
   * is "COMMENT:".
   */
  static const String commentPrefix;
  /*!
   * The PropertyMap key prefix which triggers the use of a USLT frame instead
   * of a TXXX frame for a non-standard key. In the current implementation, this
   * is "LYRICS:".
   */
  static const String lyricsPrefix;
  /*!
   * The PropertyMap key prefix which triggers the use of a WXXX frame instead
   * of a TXX frame for a non-standard key. In the current implementation, this
   * is "URL:".
   */
...
protected:
  ...
  /*!
   * Parses the contents of this frame as PropertyMap. If that fails, the
   * returned PropertyMap will be empty, and its unsupportedData() will contain
   * this frame's ID.
   */
  virtual PropertyMap asProperties() const;

  /*!
   * This helper function splits the PropertyMap \a original into three
   * ProperytMaps
   * \a singleFrameProperties, \a tiplProperties, and \a tmclProperties, such
   * that:
   * - \a singleFrameProperties contains only of keys which can be represented
   * with exactly one ID3 frame per key. In the current implementation this is
   * everything except for the fixed "involved people" keys and keys of the form
   * "TextIdentificationFrame::instrumentPrefix" + "instrument", which are
   ...
   * - \a tmclProperties contains the "musician credits" keys which should be
   * mapped to a TMCL frame
   */
  static void splitProperties(const PropertyMap &original,
