...
public:
   ...
   *    roles defined in the frame are known in
   * TextIdentificationFrame::involvedPeopleMap(), then "<role>=<name>" will be
   * contained in the returned object for each
   ...
   *  In any other case, the unsupportedData() of the returned object will
   * contain the frame's ID and, in case of a frame ID which is allowed to
   * appear more than once, the description, separated by a "/".
   *
   */
  ...
  bool setComplexProperties(const String &key,
                            const List<VariantMap> &value) override;
